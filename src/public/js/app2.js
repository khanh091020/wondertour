...
$(document).ready(function () {
  ...
  $("#start_chat").click((e) => {
    if ($("#fullName").val() === "" || $("#email").val() === "") return;
    let data = {
      name: $("#fullName").val(),
      email: $("#email").val(),
  ...
  socket.on("update_message", (data) => {
    ...
    if (!check) {
      ...
      if (notify_div.hasClass("display__none"))
        notify_div.removeClass("display__none");
      count_mess++;
    } else {
      count_mess = 1;
      if (!notify_div.hasClass("display__none"))
        notify_div.addClass("display__none");
    }
  });
...
$("#numberAdult").change(function () {
  ...
  totalPrice.html(
    "$" +
      (getNumberFromString(totalPriceAdult.text()) +
        getNumberFromString(totalPriceChildren.text()))
  );
});
...
$("#btn__completeBooking").click(function (e) {
  ...
  $("input[name='totalPrice']").val(
    getNumberFromString($("#totalPrice").text())
  );
...
$("#numberChildren").change(function () {
  totalPriceChildren.html(
    "$" + $(this).val() * (onePrice.val() - (onePrice.val() * 20) / 100)
  );
  totalPrice.html(
    "$" +
      (getNumberFromString(totalPriceAdult.text()) +
...
$("#showSuccessToast").click(function () {
  $(".photo__cus-phone").html(
    "Our staff will call your phome number <b>" +
      $("#photo__cus-phone-input").val() +
      "</b> to confirm. Thank you !"
  );
